...
module.exports = function (router, auth, util) {
    router.post('/answers', auth.isAuthenticated('admin'), util.isValidBody(answerSchema, [questionSchema, questionConditionSchema]), util.fromBodyToData, answerController.createAnswer);
    router.get('/answers/:id', auth.isAuthenticated(['admin', 'user']), util.isValidId, answerController.readAnswer);
    router.get('/answers', auth.isAuthenticated(['admin', 'user']), answerController.listAnswer);
    router.put('/answers/:id', auth.isAuthenticated('admin'), util.isValidBody(answerSchema, [questionSchema, questionConditionSchema]), util.fromBodyToData, answerController.updateAnswer);
